...
class RewardControllerTest {
    ...
    void getByName() throws Exception {
        when(rewardManager.findByName(anyString())).thenReturn(Optional.empty());

        RequestBuilder requestBuilder = MockMvcRequestBuilders
                .get("/reward/{name}", rewardsList().get(0).getName())
                .accept(MediaType.APPLICATION_JSON);

        MvcResult result = mockMvc.perform(requestBuilder).andReturn();
        MockHttpServletResponse response = result.getResponse();
        String actualResponseBody = result.getResponse().getContentAsString();
        String expectedResponseBody = "Bad brand";

        assertEquals(HttpStatus.NOT_FOUND.value(), response.getStatus());
        assertEquals(actualResponseBody, expectedResponseBody);

        verify(rewardManager, times(1)).findByName(anyString());
    }

    @Test
    void getByName_notFound() throws Exception {
        when(rewardManager.findByName(anyString())).thenReturn(Optional.ofNullable(rewardsList().get(0)));
