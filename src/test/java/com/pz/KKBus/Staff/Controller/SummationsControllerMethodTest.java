...
class SummationsControllerMethodTest {
    ...
    private List<Courses> coursesList() {
        ...
        courses.add(new Courses((long) 2, LocalDate.now().minusDays(1), Route.KatowiceToKrakow, LocalTime.parse("11:11"),
                carList().get(1), employeesList().get(1)));
        courses.add(new Courses((long) 3, LocalDate.now().minusDays(2), Route.KrakowToKatowice, LocalTime.parse("18:51"),
    ...
    @Test
    void fillWithDriverAndCar_returnFiveSummations() {
        ...
        List<SummationsController.SummationsModel> summationsModels = new ArrayList<>();
        summationsController.fillWithDriverAndCar(summationsModels);

        assertEquals(5, summationsModels.size());
        verify(employeesManager, times(1)).findByRole(Role.Driver);
        verify(carManager, times(1)).findAll();
    }

    @Test
    void dataMappingReport() {
        when(employeesManager.findByRole(Role.Driver)).thenReturn(employeesList().subList(1,3));
        when(carManager.findAll()).thenReturn(carList());

        List<Report> ktToKrk = reportList().subList(0, 2);
        List<Report> krkToKt = reportList().subList(2, 3);

        List<SummationsController.SummationsModel> finish = summationsController.dataMappingReport(ktToKrk, krkToKt);

        assertEquals(5, finish.size());
        assertEquals(3, finish.get(0).getStopPassengersPairSetModels().size());
    }
}